...
logging.getLogger("torch").setLevel(logging.ERROR)  # 仅显示 ERROR 及以上级别的日志

import torch
from setproctitle import setproctitle

from dl.data import build_dataloader, build_dataset
from dl.data.data_sampler import EnlargedSampler
from dl.data.prefetch_dataloader import CPUPrefetcher, CUDAPrefetcher
...
from dl.utils.options import copy_opt_file, dict2str, parse_options
...
def train_pipeline(root_path):
    ...
    if "DEBUG" in opt["name"]:
        # opt["datasets"]["train"]["batch_size_per_gpu"] = 2
        opt["val"]["val_freq"] = 50
        opt["logger"]["print_freq"] = 1
        opt["logger"]["save_checkpoint_freq"] = 50
        opt["logger"]["tb_logger"] = False
        # del it to avoid using wandb
        if opt["logger"].get("wandb"):
            del opt["logger"]["wandb"]
