...
from dl.utils import img2tensor
...
class svBRDF:
    ...
    def fullRandomLightsSurface(self):
        if self.nbRendering == 1:
            ...
            currentLightPos = torch.cat(
                [centerPos, torch.ones([self.nbRendering, 1]) * lightDistance], axis=-1
            )
        else:
            ...
            currentLightPos = torch.cat(
                [currentLightPos, torch.ones([self.nbRendering, 1]) * lightDistance],
                axis=-1,
            )
    ...
    def fullRandomCosine(self, n_diff=3, n_spec=6, batch_size=0, surface_size=256):
        ...
        wo_d = torch.tile(
            currentViewPos_diff.unsqueeze(-1).unsqueeze(-1),
            (1, 1, 1, surface_size, surface_size),
        )
        wi_d = torch.tile(
            currentLightPos_diff.unsqueeze(-1).unsqueeze(-1),
            (1, 1, 1, surface_size, surface_size),
        )

        # specular
        currentViewPos_spec = self.random_dir_cosine(n=n_spec, batch=batch_size)
        currentLightPos_spec = (
            currentViewPos_spec
            * torch.from_numpy(np.array([[[-1, -1, 1]]], dtype=np.float32)).cuda()
        )
        currentShift = torch.cat(
            [
                torch.empty([batch_size, n_spec, 2], dtype=torch.float32).uniform_(-1.0, 1.0),
                torch.zeros([batch_size, n_spec, 1]) + 0.0001,
            ],
            axis=-1,
        ).cuda()

        gaussian_view = torch.exp(
            torch.empty([batch_size, n_spec, 1], dtype=torch.float32).normal_(0.5, 0.75)
        ).cuda()
        gaussian_light = torch.exp(
            torch.empty([batch_size, n_spec, 1], dtype=torch.float32).normal_(0.5, 0.75)
        ).cuda()
        ...
        wo_s = torch_norm(
            torch.tile(
                currentViewPos_spec.unsqueeze(-1).unsqueeze(-1),
                (1, 1, 1, surface_size, surface_size),
            )
            - surface,
            dim=2,
        ...
        wi_s = torch_norm(
            torch.tile(
                currentLightPos_spec.unsqueeze(-1).unsqueeze(-1),
                (1, 1, 1, surface_size, surface_size),
            )
            - surface,
            dim=2,
    ...
    def torch_generate(
        ...
        if multiPoint:
            pos = pos.unsqueeze(1)
            bl, nl, _ = light_pos_world.shape
            bv, nv, _ = camera_pos_world.shape
            light_pos_world = light_pos_world.reshape(bl, nl, 1, 1, 3)
            camera_pos_world = camera_pos_world.reshape(bv, nv, 1, 1, 3)
            light_pos_world = light_pos_world.permute(0, 1, 4, 2, 3).contiguous()
            camera_pos_world = camera_pos_world.permute(0, 1, 4, 2, 3).contiguous()
            dim = 2
        else:
        ...
        view_dir_world = torch_norm(camera_pos_world - pos, dim=dim)

        # pos = torch.tile(pos,[n,1,1,1])
        light_dis_square = torch.sum(torch.square(light_pos_world - pos), dim=dim, keepdims=True)

        light_dir_world = torch_norm(light_pos_world - pos, dim=dim)

        return light_dir_world, view_dir_world, light_dis_square, pos

    def torch_distant_generate(self, camera_pos_world, light_pos_world):
        # permute = self.opt['permute_channel']
        size = self.opt["size"]
        nl, _ = light_pos_world.shape
        nv, _ = camera_pos_world.shape
        ...
        view_dir_world = torch_norm(camera_pos_world, dim=1)

        view_dir_world = torch.tile(view_dir_world, [1, 1, 256, 256])

        light_dis_square = torch.sum(torch.square(light_pos_world), 1, keepdims=True)
        light_dis_square = torch.tile(light_dis_square, [1, 1, 256, 256])

        light_dir_world = torch_norm(light_pos_world, dim=1)
        light_dir_world = torch.tile(light_dir_world, [1, 1, 256, 256])

        return light_dir_world, view_dir_world, light_dis_square
    ...
    def random_dir(self, batch=0, n=1, theta_range=[0, 70], phi_range=[0, 360]):
        ...
        phi = torch.empty(shape, dtype=torch.float32).uniform_(
            phi_range[0] / 180 * np.pi, phi_range[1] / 180 * np.pi
        )
    ...
    def _render_diffuse(
        self, n, d, l, v, dis, n_xy=False, r_single=True, use_spec=True, isAmbient=False
    ):
        INV_PI = 1.0 / math.pi
        EPS = 1e-12
        eps = torch.ones_like(dis) * 1e-12
        ...
        if not isAmbient:
            if useAugmentation:
                ...
                lampIntensity = torch.abs(
                    torch.randn((ln)).normal_(mean=1.5, std=stdDevWholeBatch)
            ...
            if useAugmentation:
                lampIntensity = torch.exp(
                    torch.randn(()).normal_(mean=torch.log(0.15), std=0.5)
    ...
    def homo2sv(self, brdf):
        if len(brdf.shape) >= 2:
            n, c = brdf.shape
            svbrdf = torch.ones((n, c, self.size, self.size), dtype=torch.float32) * (
                brdf.unsqueeze(-1).unsqueeze(-1)
            )
        else:
            c = brdf.shape[0]
            svbrdf = torch.ones((c, self.size, self.size), dtype=torch.float32) * (
                brdf.unsqueeze(1).unsqueeze(1)
            )
        return svbrdf
    ...
    def generate_pos(self, view_pos, light_pos=None, r_single=True, toLDR=None, device="cpu"):
        ...
        light_dis = torch.max(
            light_dir[:, 2:3],
            torch.mean(torch.ones_like(light_dir), 2, keepdim=True) * 0.001,
        )
    ...
    def render_from_pos(
        self, svbrdf, view_pos, light_pos=None, r_single=True, toLDR=None, device="cpu"
    ):
        if toLDR is None:
            toLDR = self.opt["toLDR"]
    ...
    def render(
        ...
        if light_dir is None:
            ...
            if obj_pos is None and single_pos:
                ...
                obj_pos[:, 2] = 0.0
            light_dir, view_dir, light_dis, surface = self.torch_generate(
                view_pos, light_pos, pos=obj_pos
            )
...
def rand_n(range_theta, range_phi, nb):
    ...
    theta = np.random.uniform(
        (range_theta[0] / 180 * np.pi), (range_theta[1] / 180 * np.pi), (nb, 1)
    )
...
def render_np(inputs, wlv, roughness_factor=0.0):
    l = wlv[0]
    v = wlv[1]
    dis = wlv[2]
    INV_PI = 1.0 / math.pi
...
def recover_wlvs_np(filename, scale_size):
    # wlvs: world light and view vectors
